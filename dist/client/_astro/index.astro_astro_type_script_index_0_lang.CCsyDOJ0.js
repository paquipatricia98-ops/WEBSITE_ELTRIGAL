function U(n,t){return new Intl.NumberFormat(t==="es"?"es-US":"en-US",{style:"currency",currency:"USD"}).format(n/100)}document.addEventListener("DOMContentLoaded",async()=>{const n=document.getElementById("menu-container");if(!n)return;const t=n.dataset.locale,k=n.dataset.api,o=document.getElementById("product-search-input"),i=document.getElementById("clear-search-btn"),h=document.getElementById("reset-filter-btn"),w=document.getElementById("category-pills-container"),v=document.getElementById("products-grid"),b=document.getElementById("product-count-label"),$=document.getElementById("no-results-state");let s="todas",u=[];try{const g=await fetch(`${k}/public/products?limit=100`);if(!g.ok)throw new Error("API Error");const p=(await g.json()).data||[],I=Array.from(new Set(p.map(e=>e.primaryCategory?.id))).filter(Boolean),A=[{id:"todas",label:"Todas las Categorías",icon:"✨"},...I.map(e=>{const r=p.find(d=>d.primaryCategory?.id===e),a=r?.primaryCategory?.slug?.[t]||e;return{id:a,label:r?.primaryCategory?.name?.[t]||"Categoría",icon:a.includes("pastel")||a.includes("dulce")?"🍰":"🥖"}})];w&&(w.innerHTML=A.map(e=>`
        ...
        `).join(""));const B=p.map(e=>{const r=e.media?.find(S=>S.isPrimary)||e.media?.[0],a=r?r.secureUrl:"",d=r&&(r.alt?.[t]||r.altText?.[t]||e.name?.[t])||"",c=t==="es"?`/es/productos/${e.slug[t]||e.slug.es}`:`/en/products/${e.slug[t]||e.slug.en}`,x=e.priceLabel?.[t]?e.priceLabel[t]:e.basePriceCents?U(e.basePriceCents,t):null,y=e.primaryCategory?.name[t]||"",P=e.primaryCategory?.slug?.[t]||"todas",L=e.shortDescription?.[t]||"",j=e.tags?.join(" ")||"",N=`${e.name[t]} ${L} ${j}`.toLowerCase();return`
            <div class="product-item-wrapper w-full sm:w-[280px] md:w-[300px] xl:w-[320px] flex-shrink-0" data-category="${P}" data-search="${N}">
