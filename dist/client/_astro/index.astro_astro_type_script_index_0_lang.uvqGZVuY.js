function H(c,t){return new Intl.NumberFormat(t==="es"?"es-US":"en-US",{style:"currency",currency:"USD"}).format(c/100)}async function w(){const c=document.getElementById("menu-container");if(!c)return;const t=c.dataset.locale,B=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"/api/v1":c.dataset.api||"/api/v1",n=document.getElementById("product-search-input"),u=document.getElementById("clear-search-btn"),v=document.getElementById("reset-filter-btn"),C=document.getElementById("category-pills-container"),$=document.getElementById("products-grid"),p=document.getElementById("product-count-label"),E=document.getElementById("no-results-state");let i="todas",f=[];const s=(o,L)=>o?typeof o=="string"?o:typeof o=="object"?o[L]||o.es||o.en||"":String(o):"";try{const o=await fetch(`${B}/public/products?limit=100`);if(!o.ok)throw new Error("API Error");const y=(await o.json()).data||[],P=Array.from(new Set(y.map(e=>e.primaryCategory?.id||e.primaryCategory?._id))).filter(Boolean),N=[{id:"todas",label:t==="es"?"Todas las Categorías":"All Categories",icon:"✨"},...P.map(e=>{const r=y.find(d=>d.primaryCategory?.id===e||d.primaryCategory?._id===e),a=s(r?.primaryCategory?.slug,t)||String(e),l=s(r?.primaryCategory?.name,t)||(t==="es"?"Categoría":"Category"),g=a.includes("pastel")||a.includes("cake")||a.includes("dulce")||a.includes("sweet")?"🍰":"🥖";return{id:a,label:l,icon:g}})];C&&(C.innerHTML=N.map(e=>`
            <button
              type="button"
            data-category="${e.id}"
        ...
        `).join(""));const j=y.map(e=>{const r=e.media?.find(M=>M.isPrimary)||e.media?.[0],a=r?r.secureUrl||r.url||"":e.productoImage||"",l=s(e.name||e.productoName,t),g=r&&s(r.altText||r.alt,t)||l,d=s(e.slug||e.productoSlug,t),m=t==="es"?`/es/productos/${d}`:`/en/products/${d}`,S=e.priceLabel?.[t]?e.priceLabel[t]:e.basePriceCents?H(e.basePriceCents,t):e.productoPrice?`$${e.productoPrice.toFixed(2)}`:null,U=s(e.primaryCategory?.name,t),T=s(e.primaryCategory?.slug,t)||"todas",I=s(e.shortDescription||e.description||e.productoDescription,t),D=Array.isArray(e.tags)?e.tags.join(" "):"",q=`${l} ${I} ${D}`.toLowerCase();return`
