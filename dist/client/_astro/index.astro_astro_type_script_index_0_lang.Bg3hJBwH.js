function M(c,t){return new Intl.NumberFormat(t==="es"?"es-US":"en-US",{style:"currency",currency:"USD"}).format(c/100)}document.addEventListener("DOMContentLoaded",async()=>{const c=document.getElementById("menu-container");if(!c)return;const t=c.dataset.locale,I=c.dataset.api,n=document.getElementById("product-search-input"),u=document.getElementById("clear-search-btn"),w=document.getElementById("reset-filter-btn"),v=document.getElementById("category-pills-container"),$=document.getElementById("products-grid"),x=document.getElementById("product-count-label"),C=document.getElementById("no-results-state");let i="todas",f=[];const s=(o,E)=>o?typeof o=="string"?o:typeof o=="object"?o[E]||o.es||o.en||"":String(o):"";try{const o=await fetch(`${I}/public/products?limit=100`);if(!o.ok)throw new Error("API Error");const y=(await o.json()).data||[],B=Array.from(new Set(y.map(e=>e.primaryCategory?.id||e.primaryCategory?._id))).filter(Boolean),P=[{id:"todas",label:t==="es"?"Todas las Categorías":"All Categories",icon:"✨"},...B.map(e=>{const r=y.find(d=>d.primaryCategory?.id===e||d.primaryCategory?._id===e),a=s(r?.primaryCategory?.slug,t)||String(e),l=s(r?.primaryCategory?.name,t)||(t==="es"?"Categoría":"Category"),g=a.includes("pastel")||a.includes("cake")||a.includes("dulce")||a.includes("sweet")?"🍰":"🥖";return{id:a,label:l,icon:g}})];v&&(v.innerHTML=P.map(e=>`
            <button
              type="button"
            data-category="${e.id}"
