import { e as createAstro, f as createComponent, k as renderComponent, r as renderTemplate, m as maybeRenderHead, h as addAttribute } from '../../../chunks/astro/server_DYzyEL0a.mjs';
import 'piccolore';
import { $ as $$BaseLayout, u as useTranslations, a as $$DoorDashButton, b as $$UberEatsButton } from '../../../chunks/BaseLayout_CmgIPuhw.mjs';
import { $ as $$Breadcrumbs } from '../../../chunks/Breadcrumbs_sYitnpyE.mjs';
import { $ as $$CloudinaryImage } from '../../../chunks/CloudinaryImage_CKpxiDzb.mjs';
import { f as formatPriceCents, $ as $$ProductCard } from '../../../chunks/ProductCard_BS--r9Mz.mjs';
import { f as getProductBySlug, h as getRelatedProducts } from '../../../chunks/publicApi_BrKjFmYz.mjs';
import { b as getProductJsonLd, c as getBreadcrumbJsonLd } from '../../../chunks/jsonld_BtTGeA5O.mjs';
...
const $$productSlug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  if (product.slug.en && productSlug !== product.slug.en) {
    return Astro2.redirect(`/en/products/${product.slug.en}`);
  }
  const relatedProducts = await getRelatedProducts(product.slug.en || productSlug || "", locale, 3);
...
✓ ${t("product.available")} </span>` : renderTemplate`<span class="px-3 py-1 text-xs font-bold rounded-full bg-red-100 text-red-800 border border-red-300"> ${t("product.unavailable")} </span>`} </div> <!-- Product Name & Price --> <div> <h1 class="font-serif text-3xl sm:text-4xl font-extrabold text-brand-brown leading-tight"> ${product.name[locale]} </h1> <div class="mt-3 flex items-baseline space-x-3"> <span class="font-serif text-3xl font-extrabold text-brand-brown"> ${formattedBasePrice} </span> </div> </div> <!-- Description --> <p class="text-base text-brand-brown/85 leading-relaxed border-t border-brand-brown/10 pt-4"> ${product.description[locale] || product.shortDescription[locale]} </p> <!-- Ingredients (if present) --> ${product.ingredients?.[locale] && renderTemplate`<div class="bg-brand-cream-dark/50 border border-brand-gold/25 rounded-2xl px-5 py-4 space-y-1"> <h3 class="text-xs font-bold uppercase tracking-widest text-brand-brown/60">🌾 Ingredients</h3> <p class="text-sm text-brand-brown/85 leading-relaxed">${product.ingredients[locale]}</p> </div>`} <!-- Allergens (if present) --> ${product.allergens && product.allergens.length > 0 && renderTemplate`<div class="bg-brand-cream-dark/50 border border-brand-gold/25 rounded-2xl px-5 py-4 space-y-1"> <h3 class="text-xs font-bold uppercase tracking-widest text-brand-brown/60">⚠️ Allergens</h3> <p class="text-sm text-brand-brown/85 leading-relaxed">${product.allergens.join(", ")}</p> </div>`} <!-- Lead Time / Notice --> ${product.availability.leadTimeDays > 0 && renderTemplate`<div class="bg-brand-gold/15 p-4 rounded-2xl border border-brand-gold/30 text-xs sm:text-sm font-semibold text-brand-brown flex items-center space-x-3"> <span class="text-lg">🕒</span> <span> ${product.availability.customNotice?.[locale] || t("product.leadTimeDays", { days: product.availability.leadTimeDays })} </span> </div>`} <!-- Variants --> ${product.variants.length > 0 && renderTemplate`<div class="space-y-2"> <span class="block text-xs font-bold text-brand-brown uppercase tracking-wider">Available Variants:</span> <div class="grid grid-cols-1 sm:grid-cols-2 gap-3"> ${product.variants.map((variant) => renderTemplate`<div class="p-3 rounded-xl border border-brand-gold/40 bg-brand-cream-dark/50 flex justify-between items-center text-sm"> <span class="font-semibold text-brand-brown">${variant.name[locale]}</span> <span class="font-bold text-brand-brown">${formatPriceCents(variant.priceCents, locale)}</span> </div>`)} </div> </div>`} <!-- Delivery Ordering Options (DoorDash & Uber Eats) --> ${renderTemplate`<div class="space-y-3 pt-2 border-t border-brand-brown/10"> <div class="flex items-center gap-2"> <span class="text-lg">🚚</span> <h3 class="font-serif font-bold text-base text-brand-brown">Delivery available</h3> </div> <p class="text-xs text-brand-brown/60">Order online through our delivery platforms:</p> <div class="flex flex-wrap items-center gap-3"> ${renderComponent($$result2, "DoorDashButton", $$DoorDashButton, { "size": "md" })} ${renderComponent($$result2, "UberEatsButton", $$UberEatsButton, { "size": "md" })} </div> </div>`} ${product.ordering.allowCustomQuote && renderTemplate`<a${addAttribute(`/en/custom-cakes#quote`, "href")} class="block w-full py-3.5 px-4 rounded-xl bg-brand-cream border-2 border-brand-gold text-brand-brown font-bold text-sm text-center hover:bg-brand-gold/10">
...
More from ${product.primaryCategory.name[locale]} </p> <h2 class="font-serif text-xl sm:text-2xl font-bold text-brand-brown">
You might also like
</h2> </div> <a${addAttribute(`/en/menu/${product.primaryCategory.slug.en}`, "href")} class="shrink-0 text-xs font-bold text-brand-brown/60 hover:text-brand-brown border border-brand-brown/20 hover:border-brand-brown/50 px-4 py-2 rounded-full transition-all duration-200">
View all →
...
}, "/Users/user/Documents/EL TRIGAL/WEBSITE_ELTRIGAL/src/pages/en/products/[productSlug].astro", void 0);

const $$file = "/Users/user/Documents/EL TRIGAL/WEBSITE_ELTRIGAL/src/pages/en/products/[productSlug].astro";
const $$url = "/en/products/[productSlug]";
